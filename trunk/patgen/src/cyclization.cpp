...
//    instMap["comment"][0] = "";
	...
	TestWriter *pTestWriter = new StilWriter();
    pTestWriter->setAllPinsGroup(pioGroupName);
    pTestWriter->writeHeader( filestr);
//    pTestWriter->writeTiming  ( filestr, wftMap,    "default");
	pTestWriter->writePattern ( filestr, cycleData, testNames[0]);
    pTestWriter->writeFooter( filestr);
	delete pTestWriter;
    ...
    //    cyEventData.setTimescale( 1e-12);
        ...
        if(vm.count("gen-evcd")) {
            DesignWriter *pDesignWriter = new EvcdWriter();
            pDesignWriter->version = "0.1";
            pDesignWriter->comments.push_back( versionString);
            pDesignWriter->comments.push_back( "Command: \n\t" + commandLineString);


            string designFileName = testNames[0] + "_cyclized." + pDesignWriter->getFileExt();
            fstream fout( designFileName.c_str(), fstream::out);
            if(fout.good()) {
                pDesignWriter->write( fout, cyEventData);
            }
            fout.close();
            delete pDesignWriter;
        }
